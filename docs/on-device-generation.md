...
_Corrected after Stage 1 shipped. Where this section and the code differ, the code and docs/programming-evidence.md are the authority._
...
**Increment model.** Counted in the unit the gym marks its kit in, not in kilograms (`LoadStep`): a barbell from a 20 kg / 45 lb bar in 2.5 kg / 5 lb steps, so a symmetric pair of plates can make every rung; dumbbells in 2.5 kg / 5 lb **per bell** — `weightKg` is always one bell, and a two-bell movement is halved per bell when seeded; machines and cables in 5 kg / 10 lb; kettlebells only in the sizes they are cast in (4–48 kg, and the sizes American racks stock in pounds); anything else in 1.25 kg / 2.5 lb; bodyweight none. Keep the existing rule that **one increment is the floor, not a ceiling** — ACSM 2009 (PMID 19204579) recommends a 2–10% increase *triggered by* performing 1–2 reps over target; it is not a safety cap and a >10% jump is not forbidden. Where one increment exceeds ~10% of current load (16 kg kettlebell, light machine work), use **double progression**: accumulate reps to the top of the range, then take the whole jump. Do not conclude "cannot progress by load."

**Week 1 = calibration, not prescription.** No validated equation predicts a starting load from age/sex/bodyweight; the validated route is a performed rep-max (Reynolds 2006, PMID 16937972: 5RM→1RM R²=0.993 bench, and adding anthropometry/age/sex added little). So: `seed10RM = BW × C(pattern, equipment) × S × A × E`, with S = 1.00 male / 0.55 upper, 0.70 lower for female (Miller 1993, PMID 8477683), A tapering 1%/yr past 40, E = 0.65 / 1.0 / 1.3 for beginner / intermediate / advanced. **This coefficient table is an engineering heuristic with no validating study.** The card labels a weight never lifted before as an estimate. In-session correction is not shipped: a bad first guess is reseeded from what was logged, the following week.
...
**RIR overlay — not shipped.** Nothing in the app logs reps in reserve, beginners cannot self-report it accurately, and everyone under-predicts reps-to-failure the further they are from it (Steele 2017 PMID 29204323; Halperin 2022 PMID 34542869). Stage 1 therefore drives every lifter off reps actually completed. Revisit only if a per-set effort control earns its place in the logging UI; the evidence for slot-specific targets (Robinson 2024 PMID 38970765; Refalo 2023 PMID 36334240) and APRE-style adjustment (Mann 2010 PMID 20543732; Graham & Cleather 2021 PMID 31009432) stands for then.
